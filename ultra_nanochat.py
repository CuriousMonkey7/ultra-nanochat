...
class DataLoaderLite():
    def __init__(self, B, T, tokenizer, split="train", bos_token="<|endoftext|>"):
        self.B = B
        self.T = T
        self.counter = 0
        ...
        self.bos_token = tokenizer.encode(bos_token, allowed_special={'<|endoftext|>'})
    ...
    def get_batch(self):
        ...
        for _ in range(self.B):
            ...
            while remaining_batch > 0:
                ...
                valid = [x for x in self.remaining_token_list if len(x) < remaining_batch]
                if not valid:
                    row += self.bos_token
        ...
        batch_tokens = torch.tensor(batch_tokens, dtype=torch.long).reshape(self.B, self.T+1)
    ...
    def __init__(self):
        ...
        for pn, p in self.named_parameters():
            if pn.endswith('proj.weight'):
                torch.nn.init.normal_(p, mean=0.0, std=0.02 / (2 * Config.n_layers)**0.5)
...
tokenizer = tiktoken.Encoding(
    ...
    special_tokens={**base_enc._special_tokens, **new_special_tokens}
)
...
# HELPER FUNCTIONS
# ============================================================================
def run_evaluation(model, tokenizer, stage_name, step, total_steps, prompt_text, expected_output):
    print(f"\n" + "="*50)
    ...
    if stage_name == "PRETRAINING":
        ...
        prompt_tokens = [
            new_special_tokens["<|bos|>"],
            new_special_tokens["<|user_start|>"],
            *tokenizer.encode(prompt_text),
            new_special_tokens["<|user_end|>"],
            new_special_tokens["<|assistant_start|>"]
        ]
...
def render_conversation(conversation, tokenizer, max_tokens=1024):
    ...
    bos = new_special_tokens["<|bos|>"]
    user_start, user_end = new_special_tokens["<|user_start|>"], new_special_tokens["<|user_end|>"]
    assistant_start, assistant_end = new_special_tokens["<|assistant_start|>"], new_special_tokens["<|assistant_end|>"]
    python_start, python_end = new_special_tokens["<|python_start|>"], new_special_tokens["<|python_end|>"]
    output_start, output_end = new_special_tokens["<|output_start|>"], new_special_tokens["<|output_end|>"]
...
def load_sft_batch(dataset, batch_size, tokenizer, max_tokens=1024):
    ...
    for _ in range(batch_size):
        ...
        padded_tokens = tokens + [tokenizer.encode("<|eos|>", allowed_special={'<|eos|>'})[0]] * (max_tokens - len(tokens))
...
def generate_response(model, tokenizer, prompt_tokens, max_new_tokens=100, temperature=0.7):
    """Auto-regressive generation for RL sampling."""
    model.eval()
    with torch.no_grad():
        current_tokens = prompt_tokens.copy()
        for _ in range(max_new_tokens):
            if len(current_tokens) >= Config.seq_len: break
            ...
            if next_token == tokenizer.encode("<|eos|>", allowed_special={'<|eos|>'})[0]:
                break
    model.train()
...
def evaluate_gsm8k_response(ground_truth_conversation, generated_response_text):
    ...
    truth_text = " ".join([p.get("text", "") for p in asst_msg["content"]]) if isinstance(asst_msg["content"], list) else asst_msg["content"]
    
    truth_answer = extract_gsm8k_answer(truth_text)
    pred_answer = extract_gsm8k_answer(generated_response_text)
    print(f"Evaluating GSM8K Response | Truth: {truth_answer} | Predicted: {pred_answer}")
...
dataloader = DataLoaderLite(B=Config.micro_batch_size, T=Config.seq_len, tokenizer=tokenizer)
val_dataloader = DataLoaderLite(B=Config.micro_batch_size, T=Config.seq_len, tokenizer=tokenizer, split="val")
# ============================================================================
# STAGE 1: PRETRAINING 
# ============================================================================
print("\nSTAGE 1: PRETRAINING")
...
for step in range(Config.pretrain_steps):
    ...
    for _ in range(grad_accum_steps):
        ...
        with torch.autocast(device_type=Config.device, dtype=Config.precision):
            logits = model(x)
            loss = loss_fn(logits.reshape(-1, Config.vocab_size), y.view(-1)) / grad_accum_steps
        loss.backward()
    optimizer.step()
    print(f"Pretrain Step {step+1}/{Config.pretrain_steps} | Loss: {loss.item() * grad_accum_steps:.4f}")
    if (step + 1) % max(1, Config.pretrain_steps // 10) == 0:
        input_prompt, expected_output = val_dataloader.get_eval_prompt()
        run_evaluation(model, tokenizer, "PRETRAINING", step, Config.pretrain_steps, input_prompt, expected_output)
...
try:
    ...
    # Train split
    sft_dataset_train = load_dataset("HuggingFaceTB/smol-smoltalk", split="train", streaming=True).take(1000)
    sft_data_list = [{"messages": ex["messages"]} for ex in sft_dataset_train]
    
    # Validation split
    sft_dataset_val = load_dataset("HuggingFaceTB/smol-smoltalk", split="test", streaming=True).take(100)
...
for step in range(Config.sft_steps):
    ...
    for _ in range(grad_accum_steps):
        ...
        inputs, targets = load_sft_batch(sft_data_list, Config.micro_batch_size, tokenizer, Config.seq_len)
        with torch.autocast(device_type=Config.device, dtype=Config.precision):
            logits = model(inputs)
            loss = loss_fn(logits.reshape(-1, Config.vocab_size), targets.view(-1)) / grad_accum_steps
        loss.backward()
    optimizer.step()
    print(f"SFT Step {step+1}/{Config.sft_steps} | Loss: {loss.item() * grad_accum_steps:.4f}")
    if (step + 1) % max(1, Config.sft_steps // 10) == 0:
        ...
        run_evaluation(model, tokenizer, "SFT", step, Config.sft_steps, input_prompt, expected_output)
...
try:
    ...
    # Train split
    rl_dataset_raw = load_dataset("openai/gsm8k", "main", split="train", streaming=True).take(500)
    rl_data_list = [{"question": ex["question"], "answer": ex["answer"], "messages": [{"role": "user", "content": ex["question"]}, {"role": "assistant", "content": ex["answer"]}]} for ex in rl_dataset_raw]
    
    rl_dataset_test = load_dataset("openai/gsm8k", "main", split="test", streaming=True).take(50)
    rl_val_list = [{"question": ex["question"], "answer": ex["answer"]} for ex in rl_dataset_test]
    ...
    rl_data_list = [
        {
            "question": "2 + 2 =",
            "answer": "2 + 2 = 4. #### 4",
            "messages": [{"role": "user", "content": "2 + 2 ="}, {"role": "assistant", "content": "2 + 2 = 4. #### 4"}]
        }
    ] * 100
...
for step in range(Config.rl_steps):
    ...
    for _ in range(Config.micro_batch_size):
        ...
        for _ in range(Config.rl_num_samples):
            generated_tokens = generate_response(model, tokenizer, question_tokens, max_new_tokens=20)
            
            try:
                generated_text = tokenizer.decode(generated_tokens[len(question_tokens):])
            except:
                generated_text = ""
            ...
            if reward > 0:  
                ...
                x = torch.tensor([generated_tokens[:-1]], dtype=torch.long).to(Config.device)
                targets = torch.tensor([generated_tokens[1:]], dtype=torch.long).to(Config.device)
                ...
                with torch.autocast(device_type=Config.device, dtype=Config.precision):
                    ...
                    neg_log_probs = loss_fn(logits.reshape(-1, Config.vocab_size), targets.view(-1))
                    ...
                    pg_loss = (neg_log_probs * advantage) / (Config.rl_num_samples * Config.micro_batch_size)
                    pg_loss.backward()
    ...
    if (step + 1) % max(1, Config.rl_steps // 10) == 0:
        ...
        run_evaluation(model, tokenizer, "RL", step, Config.rl_steps, input_prompt, expected_output)
